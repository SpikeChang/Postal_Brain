...
class Utils:
    ...
    def makeType(self, data):
        ...
        '''
        data['分类'] = 1.2
        data.loc[data['业务种类'].astype(str).str.contains(r'快包|其他', na=False),'分类'] = 1.2
        data.loc[data['业务种类'].astype(str).str.contains(r'标快|标准EMS|国际', na=False),'分类'] = 1.5
        data.loc[data['业务种类'].astype(str).str.contains(r'法院', na=False),'分类'] = 2
        data.loc[data['业务种类'].astype(str).str.contains(r'高端政务', na=False),'分类'] = 4

        data.loc[data['邮件号'].astype(str).str.contains(r'^1.*8[0-9]$', na=False),'分类'] = 2 #增值
        data.loc[data['邮件号'].astype(str).str.contains(r'^1.*9[2-4]$', na=False),'分类'] = 2 #法院
