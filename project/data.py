...
class Data:
    ...
    def readData(self):
        ...
        print("正在读取揽收数据，投递数据...")
        if os.path.exists('output/lanshou_.csv') and os.path.exists('output/toudi_.csv'):
            return False
        else:
            self.lanshou = readCsv('output/lanshou.csv',False)
            self.toudi = readCsv('output/toudi.csv')
    ...
    def preData(self):
        ...
        # 周末记录
        # 先保存有周末记录的数据备用
        self.lanshou.to_csv('output/lanshou_week.csv', encoding='gb18030', index=False)
        self.toudi.to_csv('output/toudi_week.csv', encoding='gb18030', index=False)
        ...
        self.lanshou.to_csv('output/lanshou_.csv', encoding='gb18030', index=False)
        self.toudi.to_csv('output/toudi_.csv', encoding='gb18030', index=False)
        del self.lanshou
        del self.toudi
        print("保存完毕")

    def calCost(self, ipath='', opath='output/cost/'):
        '''
        调用cost类进行代价计算
        ...
        参数：
        ipath -- 机构坐标点文件路径
        opath -- 输出文件路径（建议： 优化之后的文件可以存到 output/optimize/ 文件夹）
        '''
        if ipath != '':
            self.select = ipath
        if opath == '':
            self.opath = 'output/cost/'
        ...
        self.cost.calRes(self.select, opath) 
