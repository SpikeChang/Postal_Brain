...
class Cost:
    def __init__(self, velocity, ratio, noToName, noToLat, noToLng, id2Name):
        ...
        self.lanshou = readCX('output/lanshou_.csv')
        self.toudi = readCX('output/toudi_.csv')
    ...
    def calSemiCost(self, x):
        ...
        if len(x1) < 2 or len(x2) < 2:
            ...
            if len(src) == 0 and len(des) == 0:
                dist = x_['dist'].values[0] * 2
            else:
                dist = (np.sum(latlng2_manhattan_distance(src, des), axis=0)) + x_['dist'].values[0] * 2
        else:
    ...
    def calCost(self, x):
        ...
        print(self.res[dep_name][road][date]['tcost'], self.res[dep_name][road][date]['tdist'])
        self.res[dep_name][road][date]['lcost'], self.res[dep_name][road][date]['ldist'] = self.calSemiCost(x_l)
        print(self.res[dep_name][road][date]['lcost'], self.res[dep_name][road][date]['ldist'])
        self.res[dep_name][road][date]['cost'], self.res[dep_name][road][date]['dist'] = self.calSemiCost(x)
        print(self.res[dep_name][road][date]['cost'], self.res[dep_name][road][date]['dist'])

    def calRes(self, ipath, opath):
        '''
        按 ipath 路径的机构点坐标文件计算代价，并将结果保存至csv文件中
        保存在 opath 文件夹下的'_man.csv'和'_all.csv'两个文件中

        参数：
        ipath -- 机构坐标点文件路径
        opath -- 输出文件路径（优化之后的文件可以存到 output/optimize/ 文件夹）
        '''
        ...
        # 更新距离信息
        self.lanshou = self.updateDist(self.lanshou, ipath)
        self.toudi = self.updateDist(self.toudi, ipath)

        # 保存揽收投递总数信息
        keys = set()
        for x in self.toudi['投递机构'].unique():
            keys.add(x)
        for x in self.lanshou['投递机构']:
            keys.add(x)
        ...
        lcosts = {} # 揽收时间代价
        dists = {} # 总距离
        with open(opath + prefix + '_man.csv','w',encoding='gb18030',newline='') as f:
            w = csv.writer(f)
            w.writerow(header)
            for key1 in self.res.keys():
                ...
                for key2 in self.res[key1].keys():
                    for key3 in self.res[key1][key2].keys():
                        ...
                        w.writerow([id2Name.get(key2), key1, noToName.get(key1), key3, temp['t0'], temp['t1'], temp['t2'],
        ...
        with open(opath + prefix + '_all.csv','w',encoding='gb18030',newline='') as f:
            w = csv.writer(f)
            w.writerow(header)
            for key in self.res.keys():
                w.writerow([key, noToName.get(key), noToLng.get(key), noToLat.get(key), costs[key], days[key], costs[key]/days[key],
